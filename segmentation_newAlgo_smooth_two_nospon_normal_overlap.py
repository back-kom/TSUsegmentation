...
def gen_regions(matrix,tArray, g_regionid, unit):      # generate regions for each block
    # t1 = datetime.now()
    ...
    for i in range(0, size):
        ...
        if v.density > 0 and v_in_range(v, unit):
...
def v_in_range(v, unit):
    check = False
    boundary = int(unit*1.2)
    x_co, y_co, z_co = v.x_coordinate, v.y_coordinate, v.z_coordinate
    if (x_co >0 and x_co < boundary) and (y_co >0 and y_co < boundary) and ((z_co >0 and z_co < boundary)):
        check = True
    return check


def delete_duplicate(M_region, regions):
    print(regions.keys())
    duplicate = []
    temp_rid = []
    for i in range(len(M_region)-1):
        vi = M_region[i]
        for j in range(i+1, len(M_region)):
            vj = M_region[j]
            if (vi.x_coordinate == vj.x_coordinate) and (vi.y_coordinate == vj.y_coordinate) and (vi.z_coordinate == vj.z_coordinate):
                if vi.regionID not in temp_rid and vj.regionID not in temp_rid:
                    i_rid = vi.regionID
                    j_rid = vj.regionID
                    temp_rid.extend([i_rid,j_rid])
                    for v in regions[j_rid]:
                        v.regionID = i_rid
                    regions[i_rid].extend(regions[j_rid])
                    regions = delete_key(j_rid,regions)
                    duplicate.append(vj)
    temp_M = []
...
def outputregion(regions, shape):       # output segment regions
    for key in regions:
        ...
        mrc_new = mrcfile.new('mrcfilestest/emd4297smooth_twice_dc_nospon_normal_param23_6_overlap/{}'.format(fname), overwrite=True)
...
for k in range(0, nz-unit, unit):
    for j in range(0, ny-unit, unit):
        for i in range(0, nx-unit, unit):
            ...
            temp_region_to_lm, temp_regions = gen_regions(temp_matrix,tArray,global_regionid, unit)
...
# rid = []
# region_to_lm = sorted(region_to_lm, key=lambda voxel:voxel.regionID)
# for v in region_to_lm:
#     rid.append(v.regionID)
# key = []
# key = regions.keys()
# print(key)
# print(rid)
region_to_lm, regions = delete_duplicate(region_to_lm, regions)
print("length of local max: " + str(len(region_to_lm)))
print("number of regions:" + str(len(regions)))
