...
def merge_pair(M_regions, regions, num_pair):
    ...
    for item in pairs:
        ...
        if a < b:
            ...
            tobedelete.append(M_regions[a])
        else:
            for v in regions[b]:
                v.regionID = a
            regions[a].extend(regions[b])
            regions = delete_key(b, regions)
            tobedelete.append(M_regions[b])
    temp_M = []
    for item in M_regions:
        if item not in tobedelete:
            temp_M.append(item)
    M_regions = temp_M
    return [M_regions, regions]
...
def outputregion(regions, shape):       # output segment regions
    ...
    for key in regions:
        ...
        mrc_new = mrcfile.new('mrcfilestest/emd4297smoothtwicesingle_spon_half/{}'.format(fname), overwrite=True)
