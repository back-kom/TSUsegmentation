...
def merge_region(M_regions, regions):
    ...
    for i in range(p-1,q-1,-1):
        gra = dict()
        for j in range(0,q):
            gra[j] = gradient(M_regions[j],M_regions[i],normal_parameter)
        k = max(gra, key = gra.get)
        k = M_regions[k].regionID
        temp = M_regions[i].regionID
        for v in regions[k]:
            v.regionID = temp
        regions[temp].extend(regions[k])
        regions = delete_key(k, regions)
    del M_regions[0:q]
    ...
    #     k = max(gra, key = gra.get)
    #     k = M_regions[k].regionID
    #     # temp = M_regions[k].regionID
    #     for v in regions[k]:
    #         v.regionID = i
    #     regions[i].extend(regions[k])
    #     todelete.append(M_regions[k])
    #     regions = delete_key(k, regions)
    # temp_M = []
    # for item in M_regions:
    #     if item not in todelete:
    #         temp_M.append(item)
    # M_regions = sorted(temp_M,key = lambda voxel:voxel.density)
    num_region = len(M_regions)
    if num_region > 12:
        num_pair = num_region - 12
...
def merge_pair(M_regions, regions, num_pair, normal_param):
    ...
    for item in pairs:
        ...
        print(a,b)
        if a < b:
            a = M_regions[a].regionID
            b = M_regions[b].regionID
            for v in regions[a]:
                v.regionID = b
            print("regionID" + str(a) + str(b))
...
def outputregion(regions, shape):       # output segment regions
    ...
    for key in regions:
        ...
        mrc_new = mrcfile.new('mrcfilestest/emd0414smooththreesingle_nospon_normal/{}'.format(fname), overwrite=True)
