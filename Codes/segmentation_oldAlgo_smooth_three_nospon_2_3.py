...
def gradient(mi, mj, param):       # calculate the gradient
    ...
    distance **= 1 / 2
    return param*(mj.density - mi.density)/distance
...
def outputregion(regions, shape):       # output segment regions
    ...
    for key in regions:
        ...
        mrc_new = mrcfile.new('mrcfilestest/emd4297smooththreesingle_nospon_normal_half/{}'.format(fname), overwrite=True)
