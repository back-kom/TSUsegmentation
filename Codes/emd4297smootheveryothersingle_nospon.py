...
def merge_region(M_regions, regions):
    ...
    temp = sorted(M_regions,
                  key=lambda voxel: voxel.density)  # mSorted=sorted(region_to_lm, key=lambda voxel: voxel.density)
    M_regions = temp
    # t2 = datetime.now()
    # delta = t2 - t1
...
def outputregion(regions, shape):  # output segment regions
    ...
    for key in regions:
        ...
        mrc_new = mrcfile.new('mrcfilestest/emd4297smootheveryothersingle_nospon/{}'.format(fname), overwrite=True)
