...
def intialize():        # initialize program
    ...
    df['region number in divide'] = "6/6"
    global_regionid = -1
...
def max_density_dif(M_regions):
    ...
    for i in range(len(M_regions)-1):
        for j in range(i+1, len(M_regions)):
            den_diff = abs(M_regions[i].density - M_regions[j].density)
...
def gen_regions(matrix,tArray, g_regionid, v_on_boundary, i_step, j_step, k_step):      # generate regions for each block
    # t1 = datetime.now()
    ...
    for v in region_to_lm:
        if on_boundary(v, matrix):
            v_copy = copy.deepcopy(v)
            v_copy.x_coordinate += i_step
            v_copy.y_coordinate += j_step
            v_copy.z_coordinate += k_step
            v_on_boundary.append(v_copy)
    return [region_to_lm, regions, v_on_boundary]
...
def merge_region(region_to_lm, regions, temp_image, n_regions):
    ...
    for i in range(0, len(region_to_lm)):
        ...
        # print(len(region_to_lm), xc, yc, zc)
        region_to_lm[i].density = imgmatrix[xc, yc, zc]
    region_to_lm = sorted(region_to_lm, key=lambda voxel: voxel.density)
...
def on_boundary(voxel, matrix):
    x_edge, y_edge, z_edge = len(matrix), len(matrix[0]), len(matrix[0][0])
    x_co, y_co, z_co = voxel.x_coordinate, voxel.y_coordinate, voxel.z_coordinate
    if (x_co == 0 or x_co == 1 or x_co == x_edge or x_co == x_edge -1) or (y_co == 0 or y_co == 1 or y_co == y_edge or y_co == y_edge - 1) or (z_co == 0 or z_co == 1 or z_co == z_edge or z_co == z_edge - 1):
        return True
...
def outputregion(regions, shape):       # output segment regions
    for key in regions:
        ...
        mrc_new = mrcfile.new('mrcfilestest/emd0414/emd0414_newalgo_overlap_nospon_normal_param23_6_10112019_rep1/{}'.format(fname), overwrite=True)
...
for k in range(0, nz, unit):
    for j in range(0, ny, unit):
        for i in range(0, nx, unit):
            ...
            temp_region_to_lm, temp_regions, v_on_boundary = gen_regions(temp_matrix,tArray,global_regionid, v_on_boundary, i, j, k)
...
for v in v_on_boundary:
    ...
    region_to_lm.append(v)
    regions[region_num]=[]
    regions[region_num].append(v)
print("length of local max: " + str(len(region_to_lm)))
print("number of regions:" + str(len(regions)))
...
df.to_csv('mrcfilestest/emd0414/emd0414_newalgo_overlap_nospon_normal_param23_6_10112019_rep1/emd0414_newalgo_overlap_6_10112019_rep1.csv')
print("done")
